...
func (p *coffeeShop) registerOrder() {
	p.lockOrder.Lock()
	defer p.lockOrder.Unlock()
	p.orderCount++
	if p.orderCount == maxOrderCount {
		log.Println("It is time to close the shop!")
		close(p.closeShop)
	}
}
...
func (p *coffeeShop) barista(name string) {
	for {
		...
		case <-p.closeShop:
			log.Printf("%s stops working. Bye!", name)
			return
		}
	}
}
...
func (p *coffeeShop) customer(name string) {
	for {
		...
		case <-p.closeShop:
			log.Printf("%s leaves the shop. Bye!", name)
			return
		}
	}
}
